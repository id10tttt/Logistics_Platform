...
class Sentinel(object):
    ...
    def main_loop(self):
        ...
        # Notify.Notification.new('start_main_loop', 'function main_loop').show()
...
"""
    ...
    def oerp_call(self, action, message=False, by_input=False):
        """
        Calls a method from Odoo Server
        """
        return self.connection.env['scanner.hardware'].scanner_call(
            self.hardware_code, action, message, 'keyboard', by_input=by_input)
    ...
    def _menu_display(self, entries, highlighted, title=None):
        ...
        word_index = entries[highlighted]
